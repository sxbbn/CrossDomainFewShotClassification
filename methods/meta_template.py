...
class MetaTemplate(nn.Module):
  def __init__(self, model_func, n_way, n_support, flatten=True, leakyrelu=False, tf_path=None, change_way=True):
    ...
    self.feature    = model_func(flatten=flatten, leakyrelu=leakyrelu)   #get the feature_enconder model
  ...
  def parse_feature(self,x,is_feature):
    ...
    if is_feature:
      ...
      z_all       = self.feature.forward(x)  #get the feature of support and query
      z_all       = z_all.view( self.n_way, self.n_support + self.n_query, -1)
    z_support   = z_all[:, :self.n_support] #split the support feature
    z_query     = z_all[:, self.n_support:] #split the query feature
